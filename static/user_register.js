...
document.getElementById('user_register_form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (user_regiser_data.status == 'Success') {
            if (user_regiser_data.status == 'Success' && user_regiser_data.redirect_url) {
                window.location.href = user_regiser_data.redirect_url
            } else {
                create_notification('Invalid Redirect during registration', 'Failed')
            }
        }
    }
    catch (err) {

...
(
    ...
    END_DATE                 DATE,
    RECORD_DELETED_FLAG      INT            DEFAULT 0,
    FOREIGN KEY(USER_ID) REFERENCES {auth_schema}.USERS(USER_ID) ON DELETE CASCADE,
...
def create_auth_audit_table(auth_schema = f"{env}T_AUTH"):
    conn = connection_pool.get_connection()
    cursor = conn.cursor()
    cursor.execute(f"""
CREATE TABLE IF NOT EXISTS {auth_schema}.AUTH_AUDIT
(
    ID                       INT            AUTO_INCREMENT PRIMARY KEY,

...
COLLATE=utf8mb4_unicode_ci;
    ...
    if processing_date_count_data[0]['RCOUNT'] == 0:
        ...
        insert_query = f"INSERT INTO {utility_schema}.PROCESSING_DATE (PROC_TYP_CD, PROCESSING_DATE, NEXT_PROCESSING_DATE, PREVIOUS_PROCESSING_DATE) VALUES(%s, %s, %s, %s)"
...
def create_user_processing_date_table(utility_schema = f"{env}T_UTIL"):
    conn = connection_pool.get_connection()
    cursor = conn.cursor()
    cursor.execute(f"""
CREATE TABLE IF NOT EXISTS {utility_schema}.USER_PROCESSING_DATE
(
    ID                       INT            AUTO_INCREMENT PRIMARY KEY,
    USER_ID                  BIGINT,
    PROC_TYP_CD              VARCHAR (100),
...
COLLATE=utf8mb4_unicode_ci;
    """)
    processing_date_count_data = fetch_queries_as_dictionaries(f"SELECT COUNT(*) AS RCOUNT FROM {utility_schema}.USER_PROCESSING_DATE;", "return_none_list")
    if processing_date_count_data[0]['RCOUNT'] == 0:
        proc_date_typ_cds = [(267153904095150, 'MF_PROC', date.today(),date.today(),date.today())
                             ,(267153904095150, 'STOCK_PROC', date.today(),date.today(),date.today())
                             ,(267153904095150, 'SIM_MF_PROC', date.today(),date.today(),date.today())
                             ,(267153904095150, 'SIM_STOCK_PROC', date.today(),date.today(),date.today())
                             ]
        insert_query = f"INSERT INTO {utility_schema}.USER_PROCESSING_DATE (USER_ID, PROC_TYP_CD, PROCESSING_DATE, NEXT_PROCESSING_DATE, PREVIOUS_PROCESSING_DATE) VALUES(%s, %s, %s, %s, %s)"
        for proc_date_typ_cd_values in proc_date_typ_cds:
            cursor.execute(insert_query, proc_date_typ_cd_values)
    conn.commit()
    cursor.close()

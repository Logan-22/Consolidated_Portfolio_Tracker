...
def execute_process_group_using_metadata(process_group_name, start_date = None, end_date = None):
    process_group_metadata_rows = fetch_queries_as_dictionaries(f"SELECT * FROM METADATA_PROCESS_GROUP WHERE PROCESS_GROUP = '{process_group_name}' AND CONSIDER_FOR_PROCESS = 1;")
    ...
    try:
        for process_group_metadata in process_group_metadata_rows:
            process_group_logs[process_group_metadata['PROCESS_NAME']] = execute_process_using_metadata(process_group_metadata['PROCESS_NAME'], start_date)
            if process_group_logs[process_group_metadata['PROCESS_NAME']]['status'] == "Failed":

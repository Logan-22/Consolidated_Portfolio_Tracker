...
ON
    CURRENT_PRICE.VALUE_DATE              = (SELECT DISTINCT PROC_DATE FROM PROCESSING_DATE WHERE PROC_TYP_CD = 'MF_PROC')
...
ON
    PREV_PRICE.VALUE_DATE                 = (SELECT DISTINCT PREV_PROC_DATE FROM PROCESSING_DATE WHERE PROC_TYP_CD = 'MF_PROC')

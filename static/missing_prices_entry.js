...
const missing_prices_response = await fetch('/api/metrics/missing_prices/', {
method: "GET"
})
...
if (missing_price['EXCHANGE_SYMBOL']){
missing_entry_list_div_inner_html += 
`
<div class = "field">
    <label for = "${missing_price['EXCHANGE_SYMBOL']}_${index}">
    <b>Missing Instrument Name</b></label>
    <input type = "text" id = "${missing_price['EXCHANGE_SYMBOL']}_${index}" value = "${missing_price['EXCHANGE_SYMBOL']}" readonly/>
</div>
<div class = "field">
...
<div class = "field">
    <label for = "${missing_price['EXCHANGE_SYMBOL']}_${missing_price['VALUE_DATE']}">
    <b>Missing Price</b></label>
    <input type = "number" id = "${missing_price['EXCHANGE_SYMBOL']}_${missing_price['VALUE_DATE']}" step="any"/>
</div>
`
}
else{
...
const missing_price = document.getElementById(`${missing_price_data['EXCHANGE_SYMBOL']}_${missing_price_data['VALUE_DATE']}`)
if (missing_price.value){
const payload = {
    'INSTRUMENT_ID'  : missing_price_data['INSTRUMENT_ID']
...
const missing_price_post_response = await fetch(`/api/metrics/missing_prices/`, {
method: 'POST',
body: form_data
})

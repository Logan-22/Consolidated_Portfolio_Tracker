...
ON
    ...
    AND PREV_DEP.PROCESSING_DATE     = (SELECT DISTINCT PREVIOUS_PROCESSING_DATE FROM {env}T_UTIL.PROCESSING_DATE WHERE PROC_TYP_CD = 'MF_PROC')
LEFT OUTER JOIN
...
ON
    ...
    AND PREV_PRICE.INSTRUMENT_ID       = PREV_DEP.INSTRUMENT_ID
WHERE
    DEP.PROCESSING_DATE = (SELECT DISTINCT PROCESSING_DATE FROM {env}T_UTIL.PROCESSING_DATE WHERE PROC_TYP_CD = 'MF_PROC')
